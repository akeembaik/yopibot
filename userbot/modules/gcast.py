...
async def gcast(event):
    ...
    if xx:
        ...
        return await edit_delete(event, "**TYPING/REPLY DULU NGENTOT**")
    kk = await edit_or_reply(event, "`SABAR DEK LAGI NGIRIM...`")
    er = 0
    done = 0
        ...
        if x.is_user and not x.entity.bot:
            ...
            if chat not in DEVS:
                try:
                    ...
                    er += 1
    await kk.edit(
        f"**UDAH DIKIRIM NOH KE** `{done}` **Chat, AMPAS EMANG GAGAL NGIRIM KE** `{er}` **chat**"
    )
